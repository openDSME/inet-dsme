...
% Initial Values
C_par_0 = [kp_AH; Ti_AH; Td_AH*10];

% Optimization
warning ('optiPID: optimization starts, please be patient ...');
C_par_opt = fminsearch (@optiPIDfun2, C_par_0);
%C_par_opt = C_par_0;
...
[y1, t, x] = lsim(P,input,t); %,x0);
[y2, t, x] = lsim(T_opt,input,t); %,x0);
...
for i = 1:length(t)
  ...
  %c = Kp*e + Ki*Ta/2*integral + (Kd/(Ta*2))*diff;
  c = Kp*e + Ki*Ta/2*integral;
  % TODO Ta
  %c = Kp*(e + (1/Ti)*(Ta/2)*integral + Td/(T));
