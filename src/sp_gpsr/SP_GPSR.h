...
class INET_API SP_GPSR : public cSimpleModule, public ILifecycle, public cListener, public NetfilterBase::HookBase
{
  private:
    // GPSR parameters
    enum RoutingStrategy {NEAREST_NEIGHBOR, STRAIGHTEST_PATH};
  ...
  private:
    ...
    const GPSROption *findGpsrOptionFromNetworkDatagram(INetworkDatagram *datagram) const { return const_cast<SP_GPSR *>(this)->findGpsrOptionFromNetworkDatagram(datagram); }

    // throws an error when not found
    GPSROption *getGpsrOptionFromNetworkDatagram(INetworkDatagram *datagram);
    const GPSROption *getGpsrOptionFromNetworkDatagram(INetworkDatagram *datagram) const { return const_cast<SP_GPSR *>(this)->getGpsrOptionFromNetworkDatagram(datagram); }
